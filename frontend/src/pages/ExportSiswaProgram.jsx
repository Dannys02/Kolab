...
export default function ExportSiswaProgram() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [siswaRes, ringkasanRes] = await Promise.all([
                ...
                axios.get(
                    "http://localhost:8000/api/export/ringkasan-program",
                    {
                        headers: { Authorization: `Bearer ${token}` }
                    }
                )
    ...
    const handleExportSemuaSiswa = () => {
        exportToCSV(
            siswaData,
            `siswa_program_${new Date().toISOString().split("T")[0]}`
        );
    };
    ...
    const handleExportByProgram = async programId => {
        ...
        try {
            ...
            exportToCSV(
                response.data.data,
                `siswa_${programId}_${new Date().toISOString().split("T")[0]}`
            );
        } catch (error) {
    ...
    // Memastikan loading screen awal muncul saat data pertama kali diambil
    if (loading && siswaData.length === 0 && ringkasanData.length === 0) {
        return (
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-800 mb-2">
                        Export Data Siswa & Program
                    </h1>
                    <p className="text-gray-600">
                        Kelola dan export data pendaftaran siswa beserta pilihan
                        program mereka
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-white rounded-lg shadow p-6 border-l-4 border-green-500">
                        <p className="text-gray-500 text-sm">
                            Sudah Memilih Program
                        </p>
                        <p className="text-3xl font-bold text-green-600">
                            {
                                siswaData.filter(
                                    s => s.program_dipilih !== "Belum Memilih"
                                ).length
                            }
                        </p>
                    </div>
                ...
                {/* Distribusi Program (Conditional Rendering) */}
                <div className="bg-white rounded-lg shadow p-6 mb-8">
                    <h2 className="text-lg font-bold text-gray-800 mb-4">
                        Distribusi Program
                    </h2>
                    {loading && ringkasanData.length === 0 ? (
                        <div className="text-center p-4 text-gray-500">
                            Memuat distribusi program...
                        </div>
                    ) : ringkasanData.length > 0 ? (
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">
                                            {program.jumlah_siswa} siswa
                                        </p>
                                    </div>
                                    <button
                                        onClick={() =>
                                            handleExportByProgram(
                                                program.pilihan_program
                                            )
                                        }
                                        disabled={
                                            loading ||
                                            program.jumlah_siswa === 0
                                        }
                        ...
                        <div className="text-center p-4 text-gray-500">
                            Tidak ada data program yang tersedia.
                        </div>
                    )}
                </div>

                {/* Data Table Siswa Terdaftar (Conditional Rendering) */}
                <div className="rounded-lg shadow overflow-hidden">
                    ...
                    {loading && siswaData.length === 0 ? (
                        ...
                        <div className="text-center p-6 text-gray-500">
                            Tidak ada data siswa yang terdaftar.
                        </div>
                    )}
                </div>
            </div>
