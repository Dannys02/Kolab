...
const UserDashboard = ({ onLogout }) => {
    ...
    useEffect(() => {
        ...
        }, 4500); // Atur loading menjadi 1.5 detik (sesuaikan)
    ...
    const LIST_PROGRAM = [
        {
            id: "reguler",
            judul: "Reguler / Youth",
            deskripsi: "Program pembinaan dasar usia 10-15 tahun.",
            harga: "Rp 50.000/bln",
    ...
    const handlePayment = async e => {
        ...
        if (keuangan.total_tagihan <= 0) {
            return alert(
                "Anda tidak memiliki tagihan aktif yang perlu dibayar saat ini. Tunggu tagihan dari Admin."
            );
        }
        ...
        if (parseInt(paymentAmount) > keuangan.total_tagihan) {
            return alert(
                `Jumlah pembayaran melebihi sisa tagihan! Sisa tagihan Anda: Rp ${parseInt(
        ...
        if (
            ...
            return alert(
                "Untuk pembayaran non-cash, mohon unggah bukti transfer."
            );
        }

        if (
            !window.confirm(
                `Konfirmasi pembayaran Rp ${parseInt(
                    paymentAmount
                ).toLocaleString()}?`
            )
        )
        ...
        try {
            ...
            console.error(error);
            alert(
                "Gagal memproses pembayaran. Cek koneksi internet atau format file bukti."
            );
        } finally {
            setIsLoading(false);
        }
    ...
    const handleDeleteAccount = async () => {
        if (!window.confirm("Yakin hapus akun? Data tidak bisa kembali."))
            return;
        setIsLoading(true);
        try {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 relative">
            ...
            {/* Sidebar (UPDATED) */}
            <aside
                className={`h-screen w-64 fixed left-0 p-6 bg-gradient-to-b from-green-600 to-blue-600 shadow-2xl transform transition-transform duration-300 z-40 flex flex-col ${
                    sidebarOpen
                        ? "translate-x-0"
                        : "-translate-x-full lg:translate-x-0"
                }`}
            >
                {/* --- LOGO SEKOLAH (flex-shrink-0 agar tidak terpotong) --- */}
                <div className="flex items-center justify-center mb-8 pt-4 flex-shrink-0">
                ...
                <nav className="space-y-2 mb-3 flex-grow overflow-y-auto pr-2 pb-12">
                    ...
                    <div>
                        ...
                        <button
                            onClick={onLogout}
                            className="w-full flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-xl font-medium shadow-sm transition-colors text-sm"
                        >
                            <svg
                                className="w-4 h-4"
                                fill="none"
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6 border border-green-100 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-gray-600 mt-1">
                                    {hasBiodata
                                        ? `Halo ${user?.name}, siap berlatih hari ini?`
                                        : "Lengkapi biodata untuk memulai!"}
                                </p>
                            </div>
                            {hasBiodata && (
                                <span className="px-4 py-2 bg-green-100 text-green-700 rounded-full text-xs whitespace-nowrap font-bold border border-green-200">
                                    SISWA AKTIF
                                </span>
                            )}
                        </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
                                    <div className="bg-blue-600 p-4 flex justify-between items-center">
                                        ...
                                        <span className="text-blue-100 text-xs bg-blue-700 px-2 py-1 rounded">
                                            Minggu Ini
                                        </span>
                                    </div>
                                    <div className="divide-y divide-gray-100">
                                        {jadwals.length === 0 ? (
                                            ...
                                            jadwals.map((item, index) => (
                                                <div className="p-4 hover:bg-blue-50 transition-colors flex flex-col sm:flex-row sm:items-center gap-4">
                                ...
                                <div className="space-y-6">
                                    {pengumumans.length > 0 ? (
                                        pengumumans
                                            ...
                                            .map(pengumuman => (
                                                <div
                                                    key={pengumuman.id}
                                                    className="bg-yellow-50 rounded-2xl border border-yellow-200 p-6 shadow-sm"
                                                >
                                    ...
                                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                        ...
                                        {keuangan.total_tagihan > 0 && (
                                            <button
                                                onClick={() =>
                                                    setActivePage("kas saya")
                                                }
                                                className="mt-3 w-full py-2 bg-red-100 text-red-600 rounded-lg text-sm font-medium hover:bg-red-200"
                                            >
                                                Bayar Sekarang
                                            </button>
                                        )}
                            ...
                            <div className="bg-white p-8 rounded-2xl shadow-lg border border-red-100 relative overflow-hidden">
                                ...
                                <div className="mb-6 text-center">
                                    ...
                                    <p className="text-gray-500 text-sm">
                                        Data ini diperlukan untuk administrasi
                    ...
                    <div className="space-y-6">
                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-green-100">
                            ...
                            <p className="text-gray-600">
                                Sesuaikan dengan minat dan posisi bermainmu.
                            </p>
                        </div>
                                ...
                                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                                    <div className="mb-2">
                                        <h3 className="text-lg font-bold text-gray-800">
                                            Informasi Pilihan Program
                                        </h3>
                                    </div>
                                    <p className="text-sm text-gray-600">
                                        Pilihan program sudah tersimpan. Halaman
                                        pemilihan tidak dapat diakses kembali
                                        oleh siswa. Jika perlu perubahan,
                                        hubungi admin.
                                    </p>
                                </div>
                            </div>
                                ...
                                {biodata.pilihan_program && (
                                    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center gap-3">
                                        ...
                                        <p className="text-sm text-yellow-800">
                                            <strong>Program saat ini:</strong>{" "}
                                            {LIST_PROGRAM.find(
                                                p =>
                                                    p.id ===
                                                    biodata.pilihan_program
                                            )?.judul || biodata.pilihan_program}
                                            . Pilih program lain untuk mengubah
                                            pilihan Anda.
                                        </p>
                                    </div>
                                )}
                                            ...
                                            <div className="flex justify-between items-center pt-4 border-t border-gray-50">
                                                ...
                                                biodata.pilihan_program ? (
                                                    <button
                                                        onClick={() =>
                                                            alert(
                                                                "Anda sudah memilih program. Halaman pemilihan tidak dapat diakses kembali."
                                                            )
                                                        }
                ...
                {activePage === "profil" && (
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-green-100">
                            ...
                            <p className="text-gray-600">
                                Informasi akun dan biodata siswa.
                            </p>
                        </div>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                <h3 className="font-bold text-gray-800 mb-6 border-b pb-2">
                                    Informasi Pribadi
                                </h3>
                            ...
                            <div className="bg-blue-600 rounded-2xl shadow-lg p-6 text-white">
                                ...
                                <button
                                    onClick={() => setActivePage("kas saya")}
                                    className="w-full py-3 bg-white text-blue-600 rounded-xl font-bold shadow-sm hover:bg-blue-50 transition-colors"
                                >
                                    Bayar Sekarang
                                </button>
                            </div>
                ...
                {activePage === "kas saya" && (
                    <div className="space-y-6">
                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-green-100">
                            ...
                            <p className="text-gray-600">
                                Bayar tagihan via Transfer/QRIS.
                            </p>
                        </div>
